...
public partial class Settings : Window
{
    ...
    public Settings()
    {
        ...
        // Virtual Screen Size
        int savedWidth = int.TryParse(Config.GetEntry("VirtualWidth"), out var vw) ? vw : 2560;
        int savedHeight = int.TryParse(Config.GetEntry("VirtualHeight"), out var vh) ? vh : 1440;
        VirtualWidth.Value = savedWidth;
        VirtualHeight.Value = savedHeight;
        SaveVirtualSize.Click += (sender, args) =>
        {
            int w = (int)(VirtualWidth.Value ?? 2560);
            int h = (int)(VirtualHeight.Value ?? 1440);
            Config.SetEntry("VirtualWidth", w.ToString());
            Config.SetEntry("VirtualHeight", h.ToString());
            Console.WriteLine($"[Settings] Virtual size saved: {w}x{h}");
        };

        // Keybinds (display current values, capture via Avalonia KeyDown)
        ...
        ChangeKeybind_StartDrawing.Click += async (sender, args) =>
        {
            ...
            var keybind = await CaptureNextKeyAsync();
    ...
    /// <summary>
    /// Capture the next key press on this window for keybind remapping.
    /// Uses Avalonia KeyDown (no SharpHook).
    /// </summary>
    private Task<KeyCode> CaptureNextKeyAsync()
    {
        ...
        void handler(object? sender, KeyEventArgs e)
        {
            ...
            var keyCode = (KeyCode)(int)e.Key;
            tcs.SetResult(keyCode);
        };
    ...
    private void MarketplaceTabsOnPropertyChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
        ...
        if (MarketplaceTabs.SelectedIndex == 0) LoadLocalThemeItems();
        if (MarketplaceTabs.SelectedIndex == 1) LoadOnlineThemeItems();
    }
    ...
    private void ImageCacheLocationClearButtonOnClick(object? sender, RoutedEventArgs e)
    {
        ...
        foreach (string cachedImage in cachedImages)
        {
            ...
            catch (Exception ex) { Utils.Log(ex.ToString()); return; }
        }
    }
    ...
    private async void LoadOnlineThemeItems()
    {
        ...
        foreach (JObject themeData in MarketplaceList)
        {
            ...
            {
                { "title", title }, { "desc", description }, { "image", image },
                { "author", author }, { "id", themeData.GetValue("id").ToString() }
            };
    ...
    private void LoadLocalThemeItems()
    {
        ...
        foreach (string theme in themes)
        {
            ...
            if (File.Exists(Path.Combine(parent, Path.GetFileNameWithoutExtension(theme) + "-Data.json")))
            {
                ...
                JObject JsonData = JObject.Parse(rawJsonData);
                if (JsonData.ContainsKey("title")) title = (string)JsonData.GetValue("title");
                if (JsonData.ContainsKey("desc")) description = (string)JsonData.GetValue("desc");
                if (JsonData.ContainsKey("author")) author = (string)JsonData.GetValue("author");
                if (JsonData.ContainsKey("image")) image = (string)JsonData.GetValue("image");
            }
    ...
    private async void OpenTheme_Click(object? sender, RoutedEventArgs e)
    {
        ...
        {
            Title = "Open Theme", FileTypeFilter = filetype, AllowMultiple = false,
    ...
    private async void SaveTheme_Click(object? sender, RoutedEventArgs e)
    {
        ...
        {
            Title = "Save Theme", FileTypeChoices = filetype,
            SuggestedStartLocation = await StorageProvider.TryGetFolderFromPathAsync(Config.ThemesPath)
        });
        if (file is not null)
        {
            ...
            _savedLocation = file.TryGetLocalPath();
        }
    }
    ...
    private void LocalThemeOnClick(object? sender, RoutedEventArgs e) => throw new NotImplementedException();
